...
function compteur() {
     ...
     app.appendChild(h3);
     let compte = [5, 4, 3, 2, 1, 0];
     let i = 0;
     
     // Fonction pour afficher le décompte
     function afficherDecompte() {
         if (i < compte.length) {
             h3.innerText = `00:00:0${compte[i]}`;
             i++;
         } else {
             clearInterval(intervalID); // Arrêtez l'intervalle lorsque le décompte est terminé
         }
     }
     
     afficherDecompte(); // Affichez la première valeur immédiatement
     let intervalID = setInterval(afficherDecompte, 1000); // Démarrez l'intervalle
  }